...
#undef NOTIFY_END
      ...
      //          fd_fork_frontier_iter_init( ctx->replay->forks->frontier, ctx->replay->forks->pool
      //          );
      //      !fd_fork_frontier_iter_done(
      //          iter, ctx->replay->forks->frontier, ctx->replay->forks->pool );
      //      iter = fd_fork_frontier_iter_next(
      //          iter, ctx->replay->forks->frontier, ctx->replay->forks->pool ) ) {
      //   cnt++;
      // }
      // if( cnt > 0 ) {
      // ulong i = 0;
      // for( fd_fork_frontier_iter_t iter = fd_fork_frontier_iter_init(
      //          ctx->replay->forks->frontier, ctx->replay->forks->pool );
      //      !fd_fork_frontier_iter_done(
      //          iter, ctx->replay->forks->frontier, ctx->replay->forks->pool );
      //      iter = fd_fork_frontier_iter_next(
      //          iter, ctx->replay->forks->frontier, ctx->replay->forks->pool ) ) {
      //   fd_fork_t * fork = fd_fork_frontier_iter_ele( iter, ctx->replay->forks->frontier,
      //   ctx->replay->forks->pool ); FD_LOG_NOTICE(("fork %lu: slot %lu", i, fork->slot));
      // }
      ...
      int has_fork           = fd_ghost_print( ctx->ghost, root, ctx->replay->bft->epoch_stake );
      if( FD_UNLIKELY( has_fork ) ) {
        FD_LOG_NOTICE( ( "[fork selection] slot %lu has fork", ctx->curr_slot ) );
      }
      ...
      //       fd_blockstore_bank_hash_query( ctx->replay->blockstore, child->slot - 10 );
      //   if( FD_UNLIKELY( !bank_hash ) ) {
      //     fd_blockstore_end_read( ctx->replay->blockstore );
      //     return;
      //   }

      //   fd_slot_hash_t smr_slot_hash = { .slot = child->slot - 10, .hash = *bank_hash };
      //   fd_blockstore_end_read( ctx->replay->blockstore );

      //   fd_ghost_node_t * root = fd_ghost_node_query( ctx->ghost, &smr_slot_hash );
      //   if( root ) {
      //     fd_ghost_print( ctx->ghost, root, ctx->replay->bft->epoch_stake );
      //   }
      // }
      // }
...
read_snapshot( void * _ctx, char const * snapshotfile, char const * incremental ) {
  ...
  if( strncmp( snapshot, "wksp:", 5 ) == 0 ) {
    ...
    if( strlen( incremental ) > 0 ) {
      ...
      ctx->epoch_ctx =
          fd_exec_epoch_ctx_join( fd_exec_epoch_ctx_new( ctx->epoch_ctx_mem, 2000000UL ) );
      fd_snapshot_load( incremental, ctx->slot_ctx, false, false, FD_SNAPSHOT_TYPE_INCREMENTAL );
    }
  }
...
init_after_snapshot( fd_replay_tile_ctx_t * ctx ) {
  ...
  if( snapshot_slot != ctx->curr_slot ) {
    ...
    // FD_LOG_NOTICE(( "detected snapshot slot %lu, prev_slot %lu", snapshot_slot,
    // ctx->slot_ctx->slot_bank.prev_slot ));
    fd_fork_t * ele = fd_fork_frontier_ele_remove(
        ctx->replay->forks->frontier, &ctx->curr_slot, NULL, ctx->replay->forks->pool );
...
after_credit( void * _ctx, fd_mux_context_t * mux_ctx ) {
  ...
  if( FD_UNLIKELY( ctx->slot_ctx->blockstore == NULL ) ) {
    ...
    if( ctx->slot_ctx->slot_bank.epoch_stakes.vote_accounts_root != NULL ) {
      ulong * stake_weights_msg =
          fd_chunk_to_laddr( ctx->stake_weights_out_mem, ctx->stake_weights_out_chunk );
      fd_stake_weight_t * stake_weights = (fd_stake_weight_t *)&stake_weights_msg[4];
      ulong               stake_weight_idx =
          fd_stake_weights_by_node( &ctx->slot_ctx->slot_bank.epoch_stakes, stake_weights );
      ...
      stake_weights_msg[2] =
          fd_epoch_slot0( &epoch_bank->epoch_schedule, stake_weights_msg[0] ); /* start_slot */
      stake_weights_msg[3] = epoch_bank->epoch_schedule.slots_per_epoch;       /* slot_cnt */
      // FD_LOG_WARNING(("Sending stake weights %lu %lu %lu %lu", stake_weights_msg[0],
      // stake_weights_msg[1], stake_weights_msg[2], stake_weights_msg[3]));
      ulong tspub = fd_frag_meta_ts_comp( fd_tickcount() );

      ulong stake_weights_sz =
          4 * sizeof( ulong ) + ( stake_weight_idx * sizeof( fd_stake_weight_t ) );
      ulong stake_weights_sig = 4UL;
      fd_mux_publish( mux_ctx,
                      stake_weights_sig,
                      ...
                      tsorig,
                      tspub );
      ctx->stake_weights_out_chunk = fd_dcache_compact_next( ctx->stake_weights_out_chunk,
                                                             stake_weights_sz,
                                                             ctx->stake_weights_out_chunk0,
                                                             ctx->stake_weights_out_wmark );
    }
...
unprivileged_init( fd_topo_t *      topo,
  ...
  ctx->slot_ctx = fd_exec_slot_ctx_join( fd_exec_slot_ctx_new( &replay_slot->slot_ctx, valloc ) );
